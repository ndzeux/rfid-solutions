...
<form on:submit|preventDefault="{e => createPost(e)}">
    <div class="inp">
    ...
    <div class="editor" bind:this="{editor}" on:keydown="{e => newLineBlock(e)}" on:click="{e => activeBlock(e)}" on:paste|preventDefault={e => pastehandle(e)}>
        <div class="block" contenteditable>Write here...</div>
    </div>
    <span class="ph" bind:this="{coverph}">Image url, will showing as cover</span>
    <div class="input" bind:this="{coverarea}" contenteditable on:input="{e => coverinput(e)}" on:focusout="{e => loadCover(e)}" on:keydown="{resetcover}"></div>
    <div class="inp">
